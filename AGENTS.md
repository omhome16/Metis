...
uv run python scripts/frontend_qa.py   # Playwright UI check; expects server on :8011 (default) — start `uv run uvicorn app.main:app --port 8011` or pass a URL; needs `uv run playwright install chromium` once; auto-registers/logs into qa@metis.local
...
- `app/api/routes/` — one router file per endpoint group; wired in `app/main.py` under `/api/v1`. Includes `POST /ask/{message_id}/feedback` (negative feedback evicts matching semantic-cache entries), `GET /evals/feedback`, `GET/PUT /settings` (runtime settings, see Env gotchas), `GET /library/reorganizations` (reorg audit log), `POST /vaults/{name}/notes` (save-answer-as-note), `POST /vaults/{name}/contradiction-report` (vault-wide scan), `GET /vaults/{name}/export` (Obsidian zip), and `/imports/{url,readwise,zotero}` connectors.
- **Auth** (`app/core/auth.py` + `app/core/security.py`): `METIS_AUTH_MODE=users` (default — JWT accounts, scrypt passwords, per-user vault ownership via `vaults.owner_id`, migration 0012; the first registered account adopts pre-accounts vaults) | `token` (legacy shared-secret gate) | `none` (tests force this in conftest). Route helpers: `get_current_user`/`CurrentUser`, `owned_vault`, `vault_scope_guard` — every vault/document/conversation/search route takes a `user` param and scopes through these.
